...
def create_rbt(omega, theta, trans):
    ...
    rot = np.concatenate((rot, np.array([[trans.item(0), trans.item(1), trans.item(2)]]).T), axis=1)
    rot = np.concatenate((rot, np.array([[0,0,0,1]])), axis=0)
    return rot
...
def find_omega_theta(R):
    ...
    omega = (1 / (2 * math.sin(theta))) * np.array([R.item(2,1) - R.item(1,2), R.item(0,2) - R.item(2,0), R.item(1,0) - R.item(0,1)]).T
    return omega, theta
...
def find_v(omega, theta, trans):
    ...
    if np.array_equal(rot, np.eye(3)):
       return trans / np.linalg.norm(trans)
        ...
        a = np.subtract(np.eye(3), rot)
        w_hat = kfs.skew_3d(omega)
        omega = omega.reshape(3,1) #this format is ridiculous -> pay attention if debugging next time
        a = a.dot(w_hat) + np.dot(omega, omega.T)*theta
        inva = inv(a)
        v = np.dot(inva, trans)
        return v
